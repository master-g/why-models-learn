...
**形状想当然。** 一个神经元：行向量乘列向量得一个数；一层：$W$ 是 $(m, n)$，行是神经元。把 $W$ 的行列意义记反，参数量账和代码形状会对不上——`nn.Linear(4, 2)` 的 `weight` 是 $(2, 4)$，不是 $(4, 2)$。动手前先标形状。
