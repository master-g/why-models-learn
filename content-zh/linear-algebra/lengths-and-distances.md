...
> ![三种距离球的轮廓对比](/assets/linear-algebra/svg/lengths-and-distances.1.svg)
...
三列的数值速度不同，但都趋向$0$。有限维空间中任意两个范数等价，所以换用$\ell_1$、$\ell_2$或$\ell_\infty$不会改变「是否收敛」这个结论，只会改变距离数值与阈值的尺度。[范数](../linear-algebra/norms/) 篇给出了有限维范数等价的结论；这里把它改写成距离，就是同一组点列拥有相同的收敛点。[^lengths-norm-equivalence]

> [!marginnote] 有限维提醒
> 上述结论依赖有限维；无限维空间中的不同范数可能给出不同的收敛概念。

[^lengths-norm-equivalence]: 这里的「等价」表示存在常数$c,C>0$，使两个范数满足$c\|x\|_a\leq\|x\|_b\leq C\|x\|_a$；参见[范数](../linear-algebra/norms/)。
