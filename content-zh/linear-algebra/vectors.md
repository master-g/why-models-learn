...
**视角一：向量是数组。** 程序员对向量不陌生:`float x[784]` 就是一个 784 维向量。这个视角完全正确，而且是向量在计算机里的实际形态——分量存进一块连续内存，按下标访问。数学在这个视角上补充了两样东西：一套**运算规则**(向量之间可以加、可以乘一个数，下一节定义)，以及一套**几何直觉**(下面马上讲)。

**视角二：向量是位移。** 在平面上，二维向量 $(3, 1)$ 可以画成一支箭头：从原点出发，向右走 3、向上走 1。关键概念是：向量表达的是**位移**而不是**位置**。这支箭头可以平移到平面上的任何地方——从 $(5, 5)$ 出发向右走 3、向上走 1，到达 $(8, 6)$——只要方向和长度不变，它就是同一个向量 $(3, 1)$。位置只有一个，位移可以施加在任何位置上。这个区别不是咬文嚼字：下一节会看到，向量加法的几何意义正是「位移首尾相接」，只有位移才能首尾相接。
...
仿照加法的逐分量精神，自然可以定义逐元素乘法：

$$
\mathbf{a} \odot \mathbf{b} = (a_1 b_1,\; a_2 b_2,\; \dots,\; a_n b_n)
$$

这有个正式名字:**Hadamard 积**。用上面的 $\mathbf{a} = (1,2,3)$、$\mathbf{b} = (4,0,-1)$:
...
**角色三：中间状态。** 数据流过网络，每一层的输出都是一个新的向量(常叫 hidden state 或表示)。词的例子最典型：一个词先变成 one-hot 向量(几万维、只有一个 1)，经过嵌入层变成几百维的稠密向量，此后在 Transformer 里层层变换，始终都是向量，见 [one-hot-and-distributed](../text-representation/one-hot-and-distributed/) 与 [embeddings](../text-representation/embeddings/)。

三种角色共享同一套加法和数乘——这就是向量是地基的原因：后面每一篇，从线性层到注意力，写的都是向量的运算。

## 失效模式与常见误区

**维度不匹配的运算没有定义。** 不同维度的向量不能相加——分量对不齐。这听起来不可能犯错，但在代码里，矩阵运算库常常用广播之类的机制静默「修复」维度错误，不报错，只让结果悄悄变错(典型症状是 loss 不下降)。数学上先养成条件反射：看到 $\mathbf{a} + \mathbf{b}$，先确认两者同维。
