...
第一个分布 $p$ 是真实结果或数据分布，第二个分布 $q$ 是分类器或语言模型给出的概率。若 $q$ 把实际发生的结果赋予很小的概率，交叉熵就会很大。第一遍先计算一个分类样本的损失；交叉熵与 KL 散度的关系以及更完整的形式性质放在后面。
...
假设一个样本的真实类别是第 2 类，模型给三个类别的概率为

$$
\boldsymbol q=(0.1,0.7,0.2)
$$
...
\begin{aligned}
\mathcal L
&=-\sum_{k=1}^3y_k\log q_k\\
&=-\log 0.7\\
&\approx0.357
\end{aligned}
$$

one-hot 标签只留下真实类别对应的那一项。若模型把真实类别的概率降到 $0.1$，损失变为 $-\log0.1\approx2.303$。训练最小化交叉熵，就是提高真实类别的模型概率。

> [!marginnote] 第一遍读法
> 先记住单个分类样本的结果 $\mathcal L=-\log q_c$。下面的 KL 分解用于解释交叉熵的理论性质，不是计算分类损失的前置。
