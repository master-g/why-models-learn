...
梯度下降用当前梯度的反方向更新参数，但「方向正确」不代表任意步长都安全。L-光滑性把梯度变化限制在一个统一尺度内，于是下降引理能给出每一步的函数值上界；再加上凸性、强凸性或其他结构，才能把单步下降累积成 $O(1/k)$、线性收敛或非凸情形下的驻点保证。本篇从更新式推导步长条件，用二次函数的特征值看清稳定性，再区分三种目标类别的结论边界。
