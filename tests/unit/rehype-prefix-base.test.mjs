...
describe('rehype-prefix-base', () => {
  it('prefixes article links produced by rewrite-algebrica', () => {
    ...
    run(tree);
    const [a] = find(tree, 'a');
    assert.equal(a.properties.href, '/why-models-learn/vectors/');
  });

  it('prefixes category links and asset images', () => {
    const tree = {
      type: 'root',
      children: [makeLink('章', '/category/linear-algebra/'), makeImg('/assets/linear-algebra/svg/x.svg')],
    };
    run(tree);
    const [a] = find(tree, 'a');
    const [img] = find(tree, 'img');
    assert.equal(a.properties.href, '/why-models-learn/category/linear-algebra/');
    assert.equal(img.properties.src, '/why-models-learn/assets/linear-algebra/svg/x.svg');
  });
  ...
  it('rewrites root / to <base>/', () => {
    ...
    run(tree);
    const [a] = find(tree, 'a');
    assert.equal(a.properties.href, '/why-models-learn/');
  });
  ...
  it('is idempotent: already-prefixed paths are not double-prefixed', () => {
    const tree = { type: 'root', children: [makeLink('向量', '/why-models-learn/vectors/')] };
    run(tree);
    const [a] = find(tree, 'a');
    assert.equal(a.properties.href, '/why-models-learn/vectors/');
  });
  ...
  it('tolerates a trailing slash in the base option', () => {
    const tree = { type: 'root', children: [makeLink('向量', '/vectors/')] };
    run(tree, { base: '/why-models-learn/' });
    const [a] = find(tree, 'a');
    assert.equal(a.properties.href, '/why-models-learn/vectors/');
  });
});
