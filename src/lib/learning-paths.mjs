...
function readMathLayers(data, outline, knownAbsent, errors) {
  ...
  const mathOutline = new Set(
    [...outline.bySlug.values()]
      .filter(({ section }) => section?.part === 'math-foundations')
      .map(({ slug }) => slug),
  );
  ...
  const expectedCounts = { core: 17, backfill: 56, reference: 21 };
  for (const [layer, expected] of Object.entries(expectedCounts)) {
    if (layers[layer].slugs.length !== expected) {
      makeError(errors, `${layer} 数量应为 ${expected}，实际为 ${layers[layer].slugs.length}`);
    }
  }
...
function buildPath(data, math, outline, knownAbsent, errors) {
  ...
  const absent = flattened.filter((entry) => !entry.available);
  if (absent.length) makeError(errors, `主线正文缺失: ${absent.map((entry) => entry.slug).join(', ')}`);
